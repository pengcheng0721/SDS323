...
We evaluate the data on 39,797 online articles published by Mashable between 2013 and 2014 and attempt to build models that successfully predict whether an article goes viral or not. More specifically, we focus on a few characteristics of online articles such as number of videos/images and stipulate that an article needs to be shared over 1400 times to be classified as 'viral'. In order to offer accurate predictions, we attempt two approaches: the classification approach and the regression approach. In classification approach, we classify the training sets into two categories by the 1400 threshold. Then, we establish a  probability model for further predictions. In regression approach, we first estimate the number of shares for each article, and based on the predicted shares, classify the articles. Both methodologies are expected to outperform the null model yet we seek to find the best model for each approach and determine which approaches are more accurate in solving classification problems. 
...
### Conclusions
In order to classify articles as "viral" and "non-viral", we approach with two distinct methodologies - classification and regression. We stipulate the threshold of a 'viral' article to be 1400. By comparing the accuracies of models with different transformations, we determine that linear probability model outperforms the logistic probability model. With the same methodology, we discover that log transformation of shares in the regression model improves the model's out-of-sample accuracy. 

Next, we compare the two best-performing models with the null model. 100 times of train/test splits are conducted. The average accuracy for the classification approach is `r accuracy%>%round(3)` and for the regression approach is `r accuracy1%>%round(3)`. While both of the final regression and classification models demonstrate decent amount of improvements over the null model accuracy of `r acc1%>%round(3)`, classification models do seem to outperform the regression model by around 4%.

The statistical intuition behind the disparity, as we suspect, is connected to the fundamental difference between the two models. Classification directly predicts a discrete class label whereas regression only predicts a continuous quantity. In classification, it doesn't matter if an article is shared over 1 million times or it's only shared 1401 times. Under both circumstances, the article would be classified as "viral". The classification method discards the information that isn't useful in mere classification. In regression models, however, the information of how many shares each article has is retained, which disrupts the goal of classifying. With large outliers in our dataset, the slope coefficients of explanatory variables are magnified. Consequently, the model tends to overestimate the exact number of shares. This leads to the high false positive rate as shown in previous discussions (`r fpr2` for regression and `r fpr1` for classification) and undermines the accuracy of the regression model predictions.
